...
module.exports = {
    ...
    execute(client, message) {
        ...
        const nextSongs = songs > 200 ? `And **${songs - 200}** other song(s)...` : `In the playlist **${songs}** song(s)...`;

        embed.setDescription(`Current ${queue.current.title}\n\n${tracks.slice(0, 5).join('\n')}\n\n${nextSongs}`);

        embed.setTimestamp();
        embed.setFooter('Music comes first - Made with heart by De Serneels', message.author.avatarURL({ dynamic: true }));
